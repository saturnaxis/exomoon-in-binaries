...
for k in range(0,4):
    ...
    print(a_sat)
    #if k == 0:
    #    print(np.min(a_sat),np.max(a_sat))
    #a_sat = np.arange(0.005,0.7,0.001)*R_H/R_p
    ...
    #ax_top.set_xticklabels(['%i'% r for r in np.arange(10,120,10)])
    
    #ax_top.plot(60*R_p/R_H,0.0123,marker='$\oplus$',color='k',ms=35)
    #print(60*R_p/R_H)
    ...
    #ax.arrow(1, 0.45, -0.025, 0., width=0.01,head_width=0.03, head_length=0.03, fc='r', ec='r',transform=ax.transAxes)
    #a_rng = np.arange(0,0.7,0.01)
    if k == 0:
        #ax.fill_between(np.arange(0,0.39,0.01),0.12,1.,color='lightgray')
        ax.fill_betweenx(np.arange(0.0,1.01,0.01),0.38,0.4,color='lightgray')
    elif k == 1:
        #ax.fill_between(np.arange(0,0.39,0.01),0.035,1.,color='lightgray')
        ax.fill_betweenx(np.arange(0.0,1.01,0.01),0.38,0.4,color='lightgray')
    elif k==2:
        #ax.fill_between(np.arange(0,0.39,0.01),0.12,1.,color='lightgray')
        ax.fill_betweenx(np.arange(0.0,1.01,0.01),0.65,0.7,color='lightgray')
    else:
        #ax.fill_between(np.arange(0,0.56,0.01),0.4,1.,color='lightgray')
        ax.fill_betweenx(np.arange(0.0,1.01,0.01),0.65,0.7,color='lightgray')
    ...
    for s in range(0,len(TTV_sig)):
        ...
        #M_sat = (2./13.)*a_crit**(13./2.)*Q_p/(3.*k2_p*T*R_p**5*np.sqrt(G/m_p))
        try:
        ...
        try:
            ...
            M_sat = 0.1
        tide_cut = np.where(np.logical_and(q_ps < (M_sat),q_ps>0.001))[0]
        #ax.plot(a_sat[tide_cut][::10]/R_H,q_ps[tide_cut][::10],'.',color='w',ms=14)
    if k == 0:
    ...
    #ax_top.tick_params(which='minor',axis='both', direction='out',length = 6.0, width = 8.0)
    if k < 2:
...
fig.savefig("../figs/TTV_sensitivity.png",bbox_inches='tight',dpi=300)
plt.close()
